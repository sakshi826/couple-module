...
const HistoryView = ({ reflections, onClose }: HistoryViewProps) => (
  <div className="min-h-[100dvh] max-w-md mx-auto bg-background flex flex-col w-full">
    <div className="flex items-center px-4 pt-4 pb-1">
      ...
      <h2 className="flex-1 text-center font-heading text-xl text-foreground pr-10">Past Reflections</h2>
    </div>

    <div className="flex-1 px-6 py-4 space-y-4 overflow-y-auto">
      {reflections.length === 0 ? (
        <p className="text-center text-muted-foreground font-body mt-20">
          No reflections yet.<br />Your entries will appear here.
        </p>
      ) : (
        reflections.map((r) => (
